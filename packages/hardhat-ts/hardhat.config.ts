...
task("mint-missing-surfers", "mint the missing surfers").setAction(
  async (_taskArgs, hre) => {
    ...
    const { address: leagueContractAddress } = await hre.deployments.get(
      "WSLFantasyLeague"
    );
    const surferParser = parseSurfers();
    const SURFER_CSV_PATH = path.join(
      __dirname,
      "./assets/missing_surfers.csv"
    );
